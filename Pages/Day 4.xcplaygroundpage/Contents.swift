...
let passphrases = String(input).split(separator: "\n")

let validPassphrases = passphrases.filter {
    $0.split(separator: " ").map { String($0) }.countForElements().filter{ (key, value) -> Bool in
        value > 1
    }.isEmpty
}
validPassphrases
validPassphrases.count


let validAnagramPassphrases = passphrases.filter {
    $0.split(separator: " ").map { String($0.sorted()) }.countForElements().filter{ (key, value) -> Bool in
        value > 1
    }.isEmpty
}
validAnagramPassphrases
validAnagramPassphrases.count
