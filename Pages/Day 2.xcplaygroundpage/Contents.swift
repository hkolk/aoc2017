...
func calculateChecksum(_ matrix:String) -> Int {
    let matrix = matrix.split(separator: "\n").map { $0.split(separator: " ").flatMap{ Int(String($0)) } }
    let accu = matrix.reduce(0, { accumulator, value in
        accumulator + ((value.max() ?? 0) - (value.min() ?? 0))
    })
    return accu
}
calculateChecksum("""
...
func calculateDivisableChecksum(_ matrix:String) -> Int {
    let matrix = matrix.split(separator: "\n").map { $0.split(separator: " ").flatMap{ Int(String($0)) } }
    let accu = matrix.reduce(0, { accumulator, value in
        var result = 0
        for x in value {
