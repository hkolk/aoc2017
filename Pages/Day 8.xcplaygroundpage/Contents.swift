...
enum Operation {
    case inc, dec
    func execute(lhs:Int, rhs:Int) -> Int {
        switch self {
        case .inc:
            return lhs + rhs
        case .dec:
...
enum CompareMethod {
    case lessThen, moreThen, equalOrLessThen, equalOrMoreThen, equal, notEqualTo, unknown
    ...
    func evaluate(lhs:Int, rhs:Int) -> Bool {
        ...
        case .moreThen:
            return lhs > rhs
        case .equalOrLessThen:
            return lhs <= rhs
        case .equalOrMoreThen:
            return lhs >= rhs
        case .equal:
            return lhs == rhs
        case .notEqualTo:
    ...
    static func convert(_ input:String) -> CompareMethod {
        switch input {
        case ">":
            return .moreThen
        case "<":
            return .lessThen
        case "<=":
            return .equalOrLessThen
        case ">=":
            return .equalOrMoreThen
        case "==":
            return .equal
        case "!=":
            return .notEqualTo
...
struct Instruction {
    ...
    func execute(_ banks:[String:Int]) -> [String:Int] {
        if(expression.evaluate(banks)) {
            let currentValue = banks[register] ?? 0
            let newValue = operation.execute(lhs:currentValue, rhs:value)
            var newBanks = banks
            newBanks[register] = newValue
            if(newValue > highestValueEncountered) {
                highestValueEncountered = newValue
            }
            return newBanks
        }
        return banks
    ...
    static func parseLine(_ line:String) -> Instruction {
        let parts = line.split(separator: " ").map { String($0) }
        let register = parts[0]
        let operation:Operation = ("inc" == parts[1]) ? .inc : .dec
        let value = Int(parts[2])!
        let expression = Expression.parseParts(parts: Array(parts[4...]))
        return Instruction(register: register, operation: operation, value: value, expression: expression)
    }
}
...
struct Expression {
    ...
    func evaluate(_ banks:[String:Int]) -> Bool {
        let registeredValue = banks[register] ?? 0
        return compareMethod.evaluate(lhs: registeredValue, rhs: self.value)
    }
    
    static func parseParts(parts:[String]) -> Expression {
        let register = parts[0]
        let compareMethod:CompareMethod = CompareMethod.convert(parts[1])
        let value = Int(parts[2])!
        return Expression(register:register, compareMethod:compareMethod, value:value)
    }
}

let instructions = input2.split(separator:"\n").map { Instruction.parseLine(String($0)) }

var highestValueEncountered = 0

var banks = [String:Int]()
for instruction in instructions {
    banks = instruction.execute(banks)
}
print(banks)
let largestValue = banks.sorted(by: {lhs, rhs in
    return lhs.value >= rhs.value
}).first
print("Largest value in register \(largestValue!)" )
print("Highest value encountered \(highestValueEncountered)")
