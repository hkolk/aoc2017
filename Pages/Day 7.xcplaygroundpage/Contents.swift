...
class Element: Hashable, Equatable, CustomStringConvertible {
    let name:String
    let weight:Int
    let childerenNames:[String]
    var childeren:[Element] = []
    weak var parent:Element?
    var totalWeight:Int = 0
    
    init(name:String, weight:Int, childerenNames:[String]) {
        self.name = name
        self.weight = weight
        self.childerenNames = childerenNames
    }
    ...
    func getTotalWeight() -> Int {
        if(totalWeight == 0) {
            totalWeight = childeren.reduce(0, { accu, element in accu + element.getTotalWeight() }) + self.weight
        }
        return totalWeight
    }
    func hasChilderen() -> Bool {
    ...
    func isBalanced() -> Bool {
        ...
        var balanced = true
        childeren.forEach{child in
            let weight = child.getTotalWeight()
            print("Child: ", child.name, " weighs ", weight)
    ...
    func findUnbalanced() -> Element? {
        let weightMap = childeren.reduce(into:[Int:[Element]](), { accu, child in
            let weight = child.getTotalWeight()
            if(accu[weight] != nil) {
                accu[weight]!.append(child)
            } else {
                accu[weight] = [child]
            }
        })
        print(weightMap)
        let unbalanced = weightMap.first { dictItem in
           return dictItem.value.count == 1
        }?.value.first
        if(unbalanced == nil) {
            return self
        } else if let unbalancedChild = unbalanced!.findUnbalanced() {
    ...
    static func parseLine(_ line:String) -> Element {
        let separators = CharacterSet(charactersIn: " ()->,")
        let parts = line.components(separatedBy: separators).filter { $0.count > 0 }
        print(parts)
        return Element(name:parts[0], weight:Int(parts[1])!, childerenNames:Array(parts[2...]))
    }
}

let elements = Set(input.split(separator:"\n").map { Element.parseLine(String($0)) })

elements.forEach{ element in
    element.childerenNames.forEach{ childName in
        if let childElement = (elements.first { $0.name == childName }) {
            element.addChild(child: childElement)
        }
    }
}

var rootElement = elements.first!
while(rootElement.parent != nil) {
    rootElement = rootElement.parent!
}
print("found root element:", rootElement)
print(rootElement.isBalanced())
let unbalanced = rootElement.findUnbalanced()
print(unbalanced, unbalanced!.weight)
print(unbalanced!.parent?.childeren.map { child in "\(child.name), \(child.getTotalWeight())" })
